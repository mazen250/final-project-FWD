...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (!email) {
    return res.status(400).send({auth: false, message: 'Email is required or malformedd.'});
  }
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (!email ) {
    return res.status(400).send({auth: false, message: 'Email is missing or malformeddd.'});
  }
